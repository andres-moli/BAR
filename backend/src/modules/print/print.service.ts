...
export class PrintService {
  ...
  private line(text: string, width = 42): string {
    return text.padEnd(width) + '\n';
  }

  private separator(char = '-', width = 42): string {
    return char.repeat(width) + '\n';
  }

  private center(text: string, width = 42): string {
    const pad = Math.max(0, Math.floor((width - text.length) / 2));
    return ' '.repeat(pad) + text + '\n';
  }

  private formatOrderLines(order: any): string {
    let output = '';
  ...
  async printOrder(orderId: string): Promise<string> {
    ...
    output += this.separator();
    if (order.notes) output += this.line(`Notes: ${order.notes}`);
    output += this.center('Thank you!');
    return output;
  }

  async printPreBill(orderId: string): Promise<string> {
    const order = await this.orderRepo.findOne({
      where: { id: orderId },
  ...
  async printInvoice(invoiceId: string): Promise<string> {
    ...
    if (invoice.name) output += this.line(`Client: ${invoice.name}`);
    output += this.separator();
    if (invoice.order) {
  ...
  async printCollectionAccount(collectionId: string): Promise<string> {
    ...
    if (collection.payments?.length) {
      ...
      for (const p of collection.payments) {
        ...
        if (p.paymentMethod) output += this.line(`  ${p.paymentMethod.name}`);
      }
      output += this.separator();
    }
    if (collection.notes) output += this.line(`Notes: ${collection.notes}`);
    output += this.center('Thank you!');
    return output;
  }
}
